...
function getPopSizes(gadgetName) {
  ...
  while ((match = regex.exec(gadgetName)) !== null) {
    ...
    for (let token of tokens) {
      token = token.trim();
      if (token === "" || token === "rt" || token === "pop" || token === "sp")
        continue;
          ...
          if (popSizes && popSizes.length > 0) {
            ...
            if (i + totalBytes <= bytes.length) {
              for (let sz of popSizes) {
                ...
                else if (sz === 2) value = (dataBytes[1] << 8) | dataBytes[0];
                else {
                  value = 0;
                  for (let j = 0; j < sz; j++) value |= dataBytes[j] << (8 * j);
                }
                items.push({
                  type: "data",
                  hexBytes: dataBytes,
...
function decompileAndSet() {
  ...
  setTimeout(() => {
    try {
      ...
      refreshOutput();
      setStatus(`<i class='fa-solid fa-check"></i> Decompiled successfully`);
    } catch (e) {
      currentItems = [];
      currentByteCount = 0;
      outputDiv.innerHTML = `<div style="color:#f28b82; padding:12px;'>// Error: ${e.message}</div>`;
      updateStats(0, 0);
      setStatus(`<i class='fa-solid fa-xmark'></i> Decompile error`, true);
    }
  }, 10);
}
...
copyBtn.onclick = () => {
  let txt =
    currentMode === "detail" ? formatSimple(currentItems) : outputDiv.innerText;
  if (
    !txt ||
...
pasteBtn.onclick = async () => {
  try {
    ...
    hexInput.value = text;
    setStatus(`<i class='fa-solid fa-thumbtack'></i> Pasted from clipboard`);
  } catch {
    setStatus(
